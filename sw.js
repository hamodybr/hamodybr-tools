...
self.addEventListener('fetch',event=>{
  ...
  if(req.mode==='navigate'){
    event.respondWith(fetch(req,{cache:'no-store'}).then(r=>{
      if(r&&r.ok&&(url.pathname.endsWith('/hamodybr-tools/')||url.pathname.endsWith('/hamodybr-tools/index.html'))){const copy=r.clone();caches.open(CACHE).then(c=>c.put(FALLBACK,copy)).catch(()=>{})}
      return r;
    }).catch(()=>caches.match(FALLBACK)));
    return;
  }
  event.respondWith(fetch(req,url.origin===self.location.origin?{cache:'no-store'}:undefined).then(r=>{
    if(r&&r.ok&&url.origin===self.location.origin){const copy=r.clone();caches.open(CACHE).then(c=>c.put(req,copy)).catch(()=>{})}
    return r;
